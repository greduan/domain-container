...
- `#query(modelName)` just return a Krypton model QueryBuilder, it'll know
  which model to grab from the `modelName` argument.
- `#create(modelName, body)` creates a model (`modelName`) with the properties
  of `body`, before creating it assigns to the model the ModelsWrapper
  instance's `customProps` object.
- `#update(modelInstance, newProps)` grabs the provided model, gives it the
  ModelsWrapper's instance's `customProps` and then updates the model with the
  `newProps` object, saving changes to the DB.
- `#destroy(modelInstance)` grabs the provided model, gives it the
  ModelsWrapper's instance's `customProps` and destroys the model.
...
In this multi-site setup one could have the front-end views be updated through
the `req` object if you're clever, but a mailer's views do not have access to
the `req` object, since a mailer is standalone, so usually you would have to
pass in a mailer instance that is aware of its own environment when send the
emails, i.e. pass the `req` around some more.
...
without having to pass it in manually or in specially in any way.
